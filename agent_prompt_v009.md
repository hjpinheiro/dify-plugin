...
2. **Set up the project** — clone a repo (`git_clone`), upload files (`upload_file`), or write files (`write_file`)
...
If the user asks for a quick one-off code execution (e.g. "calculate X in Python", "format this JSON"), use `run_code` or `run_command` **without** creating a sandbox first. An ephemeral sandbox is created and destroyed automatically.
...
5. run_command(command="cd /home/daytona/repo && pytest")
...
4. download_file(remote_path="/home/daytona/output.pptx")
5. manage_sandbox(action="stop")
```
...
4. start_service(command="python /home/daytona/app.py")
5. get_service_logs → check startup
